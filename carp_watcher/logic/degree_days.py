...
def get_degree_days_for_all_streams():
    """
    :return: A JSON formatted string of stream data including name, lat, long, and status
    """
    stream_instances = Stream.objects.all()

    for stream_instance in stream_instances:
        calculate_degree_days(stream_instance)

def calculate_degree_days(stream_ins):
    """
